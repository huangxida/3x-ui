...
func panelLatestReleaseAPIURL() string {
	return panelReleaseAPIURL("")
}

func panelReleaseAPIURL(tag string) string {
	if tag != "" {
		return "https://api.github.com/repos/" + panelRepository + "/releases/tags/" + tag
	}
...
// GetUpdateInfo checks GitHub for the latest fork release. When the dev channel
...
// StartUpdate starts the fork updater using this panel's own channel setting.
